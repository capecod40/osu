...
void BeatMap::AddEntity(const int bar, const float beat, ENTITY_TYPE type, glm::vec3 center, int index, int tracker)
{
	if (entity_tracker == tracker)
	{
		...
		if (currentBar == bar && (currentBeat >= beat - 0.04 && currentBeat <= beat + 0.04)) // not very accurate
		{
			...
			case ENTITY_TYPE::BASIC:
				game->CreateBasicCircle(center, index);
				break;
			// other cases: sliders, spinners
			}
			entity_tracker++;
		}
	}
...
{
	int tracker = 0;
	AddEntity(1, 1, ENTITY_TYPE::BASIC, glm::vec3(300.0f, 300.0f, 0.0f), 1, tracker++);
